...
% Główna struktura interfejsu API Blendera:
...
W przypadku tak rozległych możliwości rozwoju systemu, konieczne jest właściwe zdefiniowanie wymagań i dążenie do ich realizacji.
Jako, że addon musi posiadać interfejs, implementacja przy pomocy API Blendera wymaga, by był podzielony na moduły odpowiedzialne za logikę i renderowanie interfejsu. System został zaimplementowany w oparciu o wzorzec MVP z uwagi na to, że jest to struktura logicznie wynikająca ze sposobu używania API Blendera. Więcej o implementacji wzorca w podrozdziale \ref{mvp}.
System powinien posiadać wszystkie następujące funkcjonalności:
...
\item Tworzenie modelu płytki zgodnego z rzeczywistymi wymiarami
% konwersja z mil/mm
% renderowanie modelu z wymiarów warstwy outline/jakiejkolwiek innej warstwy jeżeli nie ma
\item Czytanie pliku placement w formacie .csv i wczytywanie odpowiednich modeli podzespołów z bazy modeli
% cache, smart search
...
\end{itemize}
W tym przypadku modelem jest logika modułu, widokiem - klasa odpowiedzialna za renderowanie i odbieranie danych wejściowych a Prezenter to API Blendera.


%\begin{itemize}
%\item omówienie MVP, może nie MVP tylko coś innego?...
%\item omówienie pokrótce plików projektu, podział na moduły - interfejs, logika, init
%\item klasy blednera etc
%\end{itemize}
