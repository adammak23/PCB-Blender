...
% funkcja wykonania (execute),  pobiera instancję operatora i bieżący kontekst. 
%informacje o środowisku wywoływanego operatora - bieżący wybór, aktywny obiekt itp. - są przekazywane do funkcji execute () w argumencie kontekstowym
% dziedziczenie (implementowanie) klasy Operator
%API Blendera wymaga od klasy operatora implementacji ściśle zdefiniowanych metod. Jest to rodzaj „umowy” pomiędzy skryptem a systemem rdzenia Blendera. Zgadzamy się na wdrożenie wymaganych funkcji w swojej klasie. Blender zgadza się wywoływać je w ściśle określonych okolicznościach. W programowaniu obiektowym taka lista zakontraktowanych funkcji i właściwości nazywa się „interfejsem”. Aby trochę pomóc w jego implementacji, Blender API dostarcza klasę bazową dla operatorów pochodnych, o nazwie bpy.types.Operator. W żargonie programowania obiektowego Operatoris jest tak zwaną „klasą abstrakcyjną”. Po prostu zapewnia domyślne, puste implementacje wszystkich metod wymaganych przez interfejs. Nasza klasa operatora dziedziczy tę domyślną treść z bazy (bpy.types.Operator).
