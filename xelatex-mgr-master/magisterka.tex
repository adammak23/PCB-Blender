...
\end{abstract}
% opisać więcej? jeżeli tak to każdy rozdział?
...
% angielskie skróty powinny być wielkimi literami czy może nie?

\subsection{Środowisko Visual Studio Code}
developing w VS code
fajny linker zamiast kopiowania
debuggowanie
If the selected Blender executable does not use its own Python version, no packages will be installed by default. This is to make sure that the extension does not interfere with another package manager.
